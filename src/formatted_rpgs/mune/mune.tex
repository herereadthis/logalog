...
When crossing paths with an NPC (pinces, beggers, bandits), you generally have some idea of how they're gonna react. Assuming the NPC stops to talk, the topic of their conversation can be decided by a Portent. The first thing you want to do is determine their attitude towards you, which can be done with a simple d6 roll (using advantage and disadvantage as necessary):
...
\item \textit{\textbf{Subverting Expectations:}} When you don't want to spend several minutes determining every detail, see the next subsection on TWENE.
\end{itemize}

\subsection*{\textbf{TWENE}}

Questions to the oracle are well and good when you're setting the scene for the adventure, when nothing is certain. But I don't want to spend several minutes determining every detail of a tavern. So I ask the oracle a single question: \textit{\textbf{“Is everything as expected?”}}

 \textit{``Does everything appear normal?''} and \textit{``Is there anything of interest?''} are some variations. This essentially skips all the nitty-gritty of having to figure out each detail, while still leaving room for the oracle to shine.

 These questions are best used sparingly at the start of different ``scenes'' and location changes, and should never be Likely or Unlikely in order to maximise the chance for the unexpected to occur. If the oracle answers no, roll on the Table for When Everything is Not as Expected.

\noindent\begin{tabularx}{\linewidth}{|c|X|}
\hline
\textbf{d10} & \textbf{Result} \\
\hline
1 & Increase simple element: something that on its own wouldn't make a difference to how the scene plays out, something of minimal importance (such as the paintings in a mansion's parlour or the crowd at a festival)\par\smallskip
Increasing means an existing element is somehow made bigger. An increase in scale or importance potentially turns it into something that could be added to the Plot or Entity lists\\
\hline
2 & Decrease simple element (reduced) \\
\hline
3 & Add simple element: something fluff-ish adds to the flavour instead of directly influencing the adventure \\
\hline
4 & Remove simple element \\
\hline
5 & Increase major element: something that is affecting the scene, and may even be in the Plot or Entity lists \\
\hline
6 & Decrease major element \\
\hline
7 & Add major element: something important now exists in  the scene that you weren't previously aware of (like a hidden room or a backstabbing NPC) \\
\hline
...
\textbf{Elements.} Most results on the table will mention a major or simple element. An element is anything in the scene; spacial dimensions, NPCs, lighting conditions, weather, smells, the whole shebang. If you're having trouble figuring out what element to pick, a Portent can be a great pointer. Just remember to stay within the boundary of what constitutes a major or simple element.
...
I ask the Oracle if anything about the scene is as expected, receive a No, and roll on the TWENE.
\begin{itemize}
